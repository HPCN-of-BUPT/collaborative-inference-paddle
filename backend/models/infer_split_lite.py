...
config1.set_model_from_file("freezed_model/quantized_model/quant_split_pruned_opt32.nb")
...
def infer():
    ...
    input_tensor.from_numpy(tensor_img)
    predictor.run()
    batch_outputs_temp0 = predictor.get_output(0).numpy()
    batch_outputs_temp1 = predictor.get_output(1).numpy()
    
    outputs = exe2.run(inference_program2,
            fetch_list=fetch_targets2,
            feed={feed_target_names2[0]: batch_outputs_temp0,
                                  feed_target_names2[1]: batch_outputs_temp1,
