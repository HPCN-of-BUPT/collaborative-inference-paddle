...
def add_DarkNet53_conv_body1(body_input, is_test=True):
    stages, block_func = DarkNet_cfg[53]
    stages = stages[0:3]
    blocks = []
    conv1 = conv_bn_layer(body_input, ch_out=32, filter_size=3, 
    ...
    for i, stage in enumerate(stages):
        ...
        #blocks.append(block)
        if i < len(stages): # do not downsaple in the last stage 
            downsample_ = downsample(block, ch_out=block.shape[1]*2, 
                                     is_test=is_test, 
                                     name="stage.{}.downsample".format(i))
    return block, downsample_


def add_DarkNet53_conv_body2(block, downsample_, is_test=True):
    stages, block_func = DarkNet_cfg[53]
    stages = stages[3:5]
    blocks = []
    ...
    blocks.append(block)
    for i, stage in enumerate(stages):
        block = layer_warp(block_func, downsample_, 32 *(2**(i+3)), 
                           stage, is_test=is_test, 
                           name="stage.{}".format((i+3)))
        blocks.append(block)
        if i < len(stages) - 1: # do not downsaple in the last stage 
            downsample_ = downsample(block, ch_out=block.shape[1]*2, 
                                     is_test=is_test, 
                                     name="stage.{}.downsample".format((i+3)))
    return blocks[-1:-4:-1]
