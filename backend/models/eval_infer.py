...
def eval():
    ...
    if cfg.debug:
        if not os.path.exists('output'):
            os.mkdir('output')
    place = fluid.CUDAPlace(6) if cfg.use_gpu else fluid.CPUPlace()
    exe = fluid.Executor(place)
    path = 'freezed_model'  # 'model/freeze_model'
    [inference_program1, feed_target_names1, fetch_targets1] = fluid.io.load_inference_model(dirname=path, executor=exe,
    ...
    for batch_id, batch_data in enumerate(test_reader()):
        ...
        batch_outputs = exe.run(inference_program1,
            ...
            feed={feed_target_names1[0]: img,
                  feed_target_names1[1]: img_shape},
            return_numpy=False,
