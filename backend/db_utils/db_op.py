...
def add_restriction(data):
    ...
    restriction = db_model.Restriction(delay=delay,bandwidth=bandwidth,
                              cloud_computation=cloud_computation,edge_computation=edge_computation)
    db.session.add(restriction)
    db.session.commit()

    model_divide = db_model.ModelDivide(model_id=model_id,restric_id=restriction.id)
...
def add_submodel(model_divide_id):
    edge_model = db_model.Submodel(flops=11,params=12,type=2,model_divide_id=model_divide_id)
    cloud_model = db_model.Submodel(flops=50,params=76,type=1,model_divide_id=model_divide_id)
    edge_cloud_model = db_model.Submodel(flops=61, params=120, type=0,model_divide_id=model_divide_id)
...
def add_systemtest():
    edge_model = db_model.Submodel(flops=11,params=12,type=2)
    cloud_model = db_model.Submodel(flops=50,params=76,type=1)
    edge_cloud_model = db_model.Submodel(flops=61, params=120, type=0)
...
def find_result(filename):
    system = db_model.System.query.filter(db_model.System.filename == filename).order_by(db_model.System.id.desc()).first()
